...
const CustomBarChart = ({ data, dataKeys, fills }: CustomBarChartProps) => {
  ...
  const renderCustomBar = (props: any) => {
    ...
    return <Cell cursor="pointer" key={index} opacity={isActive || activeIndex === -1 ? 1.0 : 0.7} {...restProps} />;
  };
  ...
  >
    ...
    {dataKeys.map((key, index) => (
      <Bar dataKey={key} stackId="a" key={index} fill={fills[index]} onMouseOver={onMouseOver} onMouseOut={onMouseOut}>
